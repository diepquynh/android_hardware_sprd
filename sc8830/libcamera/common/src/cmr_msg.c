...
static void *cmr_common_routine(void *client_data)
{
	...
	while (1) {
		...
		CMR_LOGV("thread 0x%x, data 0x%x, message.msg_type 0x%x, sub-type 0x%x",
			(cmr_u32)thread,
			(cmr_u32)thread->p_data,
			message.msg_type,
		...
		if (CMR_THREAD_EXIT == ret) {
			CMR_LOGV("thread exit 0x%x", (cmr_u32)thread);
			break;
		}
	}
...
cmr_int cmr_thread_create(cmr_handle *thread_handle, cmr_u32 queue_length, msg_process proc_cb, void* p_data)
{
	...
	*thread_handle = 0;
	...
	CMR_LOGV("thread 0x%X, data 0x%x", (uint32_t)thread, (uint32_t)p_data);
