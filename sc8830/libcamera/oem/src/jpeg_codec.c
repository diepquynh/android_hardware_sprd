...
static cmr_int _enc_start(cmr_handle handle, struct jpeg_codec_context *jcontext)
{
	...
	CMR_LOGV("jpeg:buf addr:0x%x, size: %d",(cmr_u32)enc_cxt_ptr->stream_buf_vir, enc_cxt_ptr->stream_real_size);
							     ...
							     cmr_u32 is_last_slice, struct jpeg_codec_context *jcontext)
{
	...
	CMR_LOGV("jpeg:dec handle 0x%x, line number %d, cpy_height %d",
			 (cmr_u32)dec_cxt_ptr,dec_cxt_ptr->cur_line_num, cpy_height);
...
static cmr_int _dec_next(cmr_handle handle, struct jpeg_codec_context  *jcontext, struct jpeg_dec_next_param *param_ptr)
{
	...
	CMR_LOGI("jpeg:handle :0x%x", (cmr_u32)handle);
...
static cmr_int _jpeg_enc_wexif(struct jpeg_enc_exif_param *param_ptr, struct jpeg_wexif_cb_param *out_ptr)
{
	...
	CMR_LOGI("jpeg:src jpeg addr 0x%x, size %d thumbnail addr 0x%x, size %d target addr 0x%x,size %d",
			 (cmr_u32)input_param.src_jpeg_buf_ptr,input_param.src_jpeg_size,
			 (cmr_u32)input_param.thumbnail_buf_ptr,input_param.thumbnail_buf_size,
			 (cmr_u32)input_param.target_buf_ptr,input_param.target_buf_size);
	...
	CMR_LOGI("jpeg:output: addr 0x%x,size %d", (cmr_u32)out_ptr->output_buf_virt_addr, (cmr_u32)out_ptr->output_buf_size);
...
static cmr_int _check_enc_start_param(struct jpeg_enc_in_param *in_parm_ptr)
{
	...
	CMR_LOGV("jpeg:phy addr 0x%x 0x%x, vir addr 0x%x 0x%x",
	...
	CMR_LOGV("jpeg:stream phy 0x%x vir 0x%x, size 0x%x",
...
cmr_int jpeg_enc_start(struct jpeg_enc_in_param *in_parm_ptr)
{
	...
	CMR_LOGV("jpeg:0x%x", (cmr_u32)enc_cxt_ptr);
	if (NULL == enc_cxt_ptr) {
		ret = JPEG_CODEC_NO_MEM;
		goto enc_start_end;
	}
	...
	jcontext->type = 0;
	CMR_LOGV("jpeg:handle 0x%x.",(cmr_u32)enc_cxt_ptr);
enc_start_end:
	if (ret) {
		if (enc_cxt_ptr) {
			free(enc_cxt_ptr);
...
cmr_int jpeg_dec_start(struct jpeg_dec_in_param *in_parm_ptr)
{
	...
	CMR_LOGV("jpeg:dec handle 0x%x", (cmr_u32)dec_cxt_ptr);
dec_start_end:
	if (ret) {
		if (dec_cxt_ptr) {
			free(dec_cxt_ptr);
...
cmr_int jpeg_enc_thumbnail(struct jpeg_enc_in_param *in_parm_ptr, cmr_uint *stream_size_ptr)
{
	...
	enc_cxt_ptr = (struct jpeg_enc *)malloc(sizeof(struct jpeg_enc));
	CMR_LOGV("jpeg:thumbnail enc: 0x%x", (cmr_u32)enc_cxt_ptr);
