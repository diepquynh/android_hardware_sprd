...
#define IS_DV_SCENE(info) (GetCameraCaptureIntent(&(info)) == CAPTURE_INTENT_VIDEO_RECORD\
					 || GetCameraCaptureIntent(&(info)) == CAPTURE_INTENT_VIDEO_SNAPSHOT)
...
int SprdCameraHWInterface2::androidPreviewFmtToDrvPrvFmt(uint32_t androidFmt, int8_t *drvFmt)
{
	int ret = 0;
	...
	m_reprocessOutputStreamId(0)
{
	...
	memset(mPreviewHeapArray_phy_dynamic, 0, sizeof(mPreviewHeapArray_phy_dynamic));
	memset(mPreviewHeapArray_vir_dynamic, 0, sizeof(mPreviewHeapArray_vir_dynamic));
	memset(mZslBufArray_vir_dynamic, 0, sizeof(mZslBufArray_vir_dynamic));
	memset(mZslBufArray_phy_dynamic, 0, sizeof(mZslBufArray_phy_dynamic));
	memset(mZslBufIndex_dynamic, 0, sizeof(mZslBufIndex_dynamic));
	memset(mPreviewBufIndex_dynamic, 0, sizeof(mPreviewBufIndex_dynamic));
	...
	memset(&m_previewSize, 0, sizeof(cam_size));
	memset(&m_captureSize, 0, sizeof(cam_size));
	...
	m_Camera2 = camera;
	if (CMR_CAMERA_SUCCESS != camera_init(cameraId, camera_cb, this, 0, &mCameraHandle)) {
	...
	//SET_PARM(mCameraHandle, CAMERA_PARM_HAL_MODE,(uint32_t)HAL_MODE_STREAM);
...
void SprdCameraHWInterface2::release()
{
	...
	if (camStatus != SPRD_IDLE) {
		...
		if (camStatus == SPRD_PREVIEW_IN_PROGRESS) {
			...
			if (res != CMR_CAMERA_SUCCESS) {
				HAL_LOGE("releaseStream error.");
			}
		}
	}
	while (0 < mSetDDRFreqCount) {
		if (set_ddr_freq(NO_FREQ_REQ)) {
			mSetDDRFreqCount = 0;
			HAL_LOGD("ddr set fail, quit yet!");
			break;
		}
	}
	...
	if (mCameraInfo[m_CameraId]) {
		free(mCameraInfo[m_CameraId]);
		mCameraInfo[m_CameraId] = NULL;
	}
	if (mVendorTagOps) {
		free(mVendorTagOps);
		mVendorTagOps = NULL;
...
int SprdCameraHWInterface2::notifyRequestQueueNotEmpty()
{
	...
	int ret;
	bool isFreeReq = true;
	bool isRecReleaseBuf = false;
	bool isReqFull = false;
	...
	if (!isRecReleaseBuf) {
		...
		if (reqNum >= MAX_REQUEST_NUM) {
			...
			if (ret == NAME_NOT_FOUND) {
				...
				if ((capture_intent)entry.data.u8[0] == CAPTURE_INTENT_STILL_CAPTURE) {
					m_ReqQueue.push_back(curReq);
					isFreeReq = false;
					HAL_LOGD("%s force process %p",__FUNCTION__, curReq);
				}
			}
			if (isFreeReq) {
				HAL_LOGD("queue is full. ignoring request");
		...
		if (curReq) {
			if (reqNum >= MAX_REQUEST_NUM) {
				...
				if (!isRecReleaseBuf) {
					...
					isRecReleaseBuf = false;
				}
			}
		}
		HAL_LOGD("%s Srv req %p",__FUNCTION__,curReq);
	}
	HAL_LOGD("srv clear flag processing=%d,recbuf=%d,reqful=%d,capreq=%d",
				m_reqIsProcess, isRecReleaseBuf, isReqFull, isFreeReq);
	if (!isRecReleaseBuf && (!isReqFull || (isReqFull && !isFreeReq))) {
		if (!m_reqIsProcess) {
...
int SprdCameraHWInterface2::getInProgressCount()
{
	...
	if (1/*!(GetCameraPictureMode() == CAMERA_NORMAL_CONTINUE_SHOT_MODE && GetContinuPicCount() > 0)*/) {
		if (SPRD_WAITING_RAW == getCaptureState() || SPRD_WAITING_JPEG == getCaptureState()) {
			ProcNum++ ;
			if (isFirstEnt) {
				isFirstEnt = false;
				mWaitPicTime = start_timestamp;
			} else {
				start_timestamp -= mWaitPicTime;
				if (start_timestamp > kBurstCapWaitTime) {
					HAL_LOGV("wait time =%lld over 3 second",start_timestamp);
					ProcNum--;
				}
			}
		}
...
static int initCamera2Info(int cameraId)
{
	if (cameraId == 0) {
		...
		g_Camera2[0]->sensorW              = SIZE_ALIGN(jpegResolutionSensorBack[0]);
		g_Camera2[0]->sensorH              = SIZE_ALIGN(jpegResolutionSensorBack[1]);
		g_Camera2[0]->sensorRawW           = SIZE_ALIGN(jpegResolutionSensorBack[0]);
		g_Camera2[0]->sensorRawH           = SIZE_ALIGN(jpegResolutionSensorBack[1]);
		...
		g_Camera2[1]->sensorW             = SIZE_ALIGN(jpegResolutionSensorFront[0]);
		g_Camera2[1]->sensorH             = SIZE_ALIGN(jpegResolutionSensorFront[1]);
		g_Camera2[1]->sensorRawW          = SIZE_ALIGN(jpegResolutionSensorFront[0]);
		g_Camera2[1]->sensorRawH          = SIZE_ALIGN(jpegResolutionSensorFront[1]);
		...
		g_Camera2[1]->minFocusDistance    = 0.0f;
...
#define ADD_OR_SIZE( tag, data, count ) \
	...
	int32_t cropRegion[4] = {
		0, 0, camHal->sensorW, camHal->sensorH
	};
	ADD_OR_SIZE(ANDROID_SCALER_CROP_REGION, cropRegion, 4);
	...
	static const int32_t aeTargetFpsRangeFront[2] = {
		10, 25
	};
	static const int32_t aeTargetFpsRangeBack[2] = {
		10, 30
	};
	if (m_CameraId == 1) {
		ADD_OR_SIZE(ANDROID_CONTROL_AE_TARGET_FPS_RANGE, aeTargetFpsRangeFront, 2);
	} else {
		ADD_OR_SIZE(ANDROID_CONTROL_AE_TARGET_FPS_RANGE, aeTargetFpsRangeBack, 2);
	}
	...
	static const int bufID = -1;
	ADD_OR_SIZE(ANDROID_SPRD_REC_BUF_INFO, &bufID, 1);
...
const char *SprdCameraHWInterface2::getVendorSectionName(/*get base type*/
...
int SprdCameraHWInterface2::getVendorTagType(
				const vendor_tag_query_ops_t *v,
				uint32_t tag)
{
	uint32_t tag_section = (tag >> 16) - VENDOR_SECTION;
...
const char *SprdCameraHWInterface2::getVendorTagName(
				const vendor_tag_query_ops_t *v,
				uint32_t tag)
{
	uint32_t tag_section = (tag >> 16) - VENDOR_SECTION;
...
int SprdCameraHWInterface2::getVendorTagCnt(const vendor_tag_query_ops_t *v)
{
	HAL_LOGV("dont' handle");
	return 0;
}

void SprdCameraHWInterface2::getVendorTags(
				const vendor_tag_query_ops_t *v,
				uint32_t *tag_array)
{
	HAL_LOGV("dont' handle");
...
int SprdCameraHWInterface2::Callback_AllocCapturePmem(void* handle, unsigned int size, unsigned int *addr_phy, unsigned int *addr_vir)
{
	...
	if (0 == ret) {
		*addr_vir = (unsigned int)(pHeapIon->getBase());
		if (*addr_vir == 0 || *addr_vir == 0xffffffff) {
			delete pHeapIon;
			HAL_LOGE("get virtual add %d",*addr_vir);
			return -1;
		}
...
int SprdCameraHWInterface2::Callback_FreeCapturePmem(void* handle)
{
	...
	for (i=0; i<camera->mMiscHeapNum; i++) {
		...
		if (pHeapIon != NULL) {
			...
			delete pHeapIon;
			camera->mMiscHeapArray[i].ion_heap = NULL;
		}
...
sprd_camera_memory_t* SprdCameraHWInterface2::GetCachePmem(int buf_size, int num_bufs, bool IsCach)
{
	int paddr, psize;
	...
	if (0 == ret) {
		...
		HAL_LOGD("phys_addr 0x%x, data:0x%x, phys_size: 0x%x alloc mem=%p",
		memory->phys_addr, (uint32_t)(memory->data), memory->phys_size, pHeapIon);
	}
	if (ret) {
		if (memory) {
			free(memory);
...
bool SprdCameraHWInterface2::allocateCaptureMem(void)
{
	...
	mRawHeap = GetCachePmem(buffer_size, kRawBufferCount, true);
	if (NULL == mRawHeap) {
...
void SprdCameraHWInterface2::freeCaptureMem()
{
	...
	HAL_LOGD("mRawHeap %p misc=%d",mRawHeap, mMiscHeapNum);
	if (mRawHeap) {
	...
	for (i=0; i<mMiscHeapNum; i++) {
		...
		if (pHeapIon != NULL) {
			...
			delete pHeapIon;
			mMiscHeapArray[i].ion_heap = NULL;
		}
		HAL_LOGD("free cap i=%d",i);
	}
	mMiscHeapNum = 0;
}

bool SprdCameraHWInterface2::allocatePreviewMem(cmr_u32 *size, cmr_u32 *sum, cmr_uint *phy_addr, cmr_uint *vir_addr)
{
	int i = 0;
	...
	#endif
	if (!size || !sum || !phy_addr || !vir_addr) {
		return false;
	}
	PreviewHeapNum = (int)(*sum);
	mPreviewHeapNum = PreviewHeapNum;
	buffer_size = (uint32_t)(*size);
	for (; i < PreviewHeapNum; i++) {
		HAL_LOGD("prv mem i=%d",i);
		mPreviewHeapArray[i] = GetCachePmem(buffer_size, 1, true);
		...
		phy_addr[i] = (cmr_uint)(mPreviewHeapArray[i]->phys_addr);
		vir_addr[i] = (cmr_uint)(mPreviewHeapArray[i]->data);
	}
	return true;
...
status_t SprdCameraHWInterface2::set_ddr_freq(uint32_t mhzVal)
{
	const char*     freq_in_khz = NO_FREQ_STR;
	uint32_t        tmpSetFreqCount = mSetDDRFreqCount;

	HAL_LOGV("set_ddr_freq to %d now count %d freq %d E", mhzVal, mSetDDRFreqCount, mSetDDRFreq);
	if (mhzVal == mSetDDRFreq && NO_FREQ_REQ != mhzVal) {
		HAL_LOGV("set_ddr_freq same freq %d need not set", mhzVal);
		return NO_ERROR;
	}
	...
	if (NULL == fp) {
		HAL_LOGE("set_ddr_freq Failed to open %s X", set_freq);
		return 1;
	}
	...
	switch (mhzVal) {
		...
		case BASE_FREQ_REQ:
			if (NO_FREQ_REQ == mSetDDRFreq) {
				...
				HAL_LOGV("set_ddr_freq clear freq for change!");
		...
		default:
			HAL_LOGE("set_ddr_freq unrecognize set frequency, error!");
			break;
	}
	...
	HAL_LOGV("set_ddr_freq to %skhz now count %d freq %d X", freq_in_khz, mSetDDRFreqCount, mSetDDRFreq);
	fclose(fp);
	return NO_ERROR;
}
			...
			uint32_t *max_buffers)
{
	...
	if (m_Stream[STREAM_ID_PREVIEW] == NULL) {
		...
		*format_actual = HAL_PIXEL_FORMAT_YCrCb_420_SP;
		...
		#endif
		if (m_previewSize.width != width || m_previewSize.height != height) {
			struct img_size req_size;
			m_previewSize.width = width;
			m_previewSize.height = height;
			req_size.width = (cmr_u32)m_previewSize.width;
			req_size.height = (cmr_u32)m_previewSize.height;
				...
				uint32_t *max_buffers)
{
	...
	*max_buffers = 1;/*must not equal to 0*/
	...
	if (m_captureSize.width != width || m_captureSize.height != height) {
		struct img_size req_size;
		m_captureSize.width = width;
		m_captureSize.height = height;
		req_size.width = (cmr_u32)m_captureSize.width;
		req_size.height = (cmr_u32)m_captureSize.height;
...
bool SprdCameraHWInterface2::WaitForCaptureStart()
{
	Mutex::Autolock stateLock(&mStateLock);
	int res = 0;
	bool isHDRmode = 0;/*(GetCameraPictureMode() == CAMERA_HDR_MODE);*/
...
void SprdCameraHWInterface2::HandleStopPreview(enum camera_cb_type cb,
										  void *parm4)
{
	HAL_LOGD("in,cb = %d, parm4 = 0x%lx, state = %s",
...
void SprdCameraHWInterface2::HandleEncode(camera_cb_type cb, void *parm4)
{
	HAL_LOGD("in: cb = %d, parm4 = 0x%lx, state = %s",
	...
	case CAMERA_EXIT_CB_DONE: {
			...
			if (SPRD_WAITING_JPEG == getCaptureState() || reProcMask & STREAM_MASK_JPEG) {
				...
				substream_parameters_t  *zslParms = &m_subStreams[STREAM_ID_ZSL];
				...
				if(reProcMask & STREAM_MASK_JPEG){
					...
					CancelStreamBuf(zslParms, true, STREAM_ID_ZSL);
					if(!getStreamBuffer(true, STREAM_ID_ZSL, 3)) {
						return;
					}
					EnqJpgOrZslGraphicBuf(tmpCBpara->outPtr, timeStamp, (uint16_t)STREAM_ID_JPEG);
				}else{
...
void SprdCameraHWInterface2::HandleCancelPicture(camera_cb_type cb, void *parm4)
{
...
bool SprdCameraHWInterface2::FindStreamBuf(T *stream, S type, S *array, int *rets)
{
	bool ret = false;
	int i = 0;

	for (; i < stream->numSvcBuffers ; i++) {
		if (type == array[i]) {
			ret = true;
			if (rets)
				*rets = i;
...
void SprdCameraHWInterface2::DisplayPictureImg(struct camera_frame_type *frame)
{
	status_t                res = 0;
	...
	buffer_size <<= 1;
	#endif
	cam_Add = GetCachePmem(buffer_size, 1, false);
	...
	if ( 0 != camera_get_data_redisplay(cam_Add->phys_addr, targetStreamParms->width, targetStreamParms->height, frame->y_phy_addr,
									frame->uv_phy_addr, frame->dx, frame->dy)) {
	...
	if (!m_IsNeedHalAllocPrvBuf) {
		if (!FindStreamBuf(targetStreamParms, *buf, (buffer_handle_t *)targetStreamParms->svcBufHandle, &Index))
			goto cancel_buf;
	}
	if (targetStreamParms->svcBufStatus[Index] != ON_SERVICE
...
void SprdCameraHWInterface2::HandleTakePicture(camera_cb_type cb, void *parm4)
{
	...
	case CAMERA_RSP_CB_SUCCESS:
		if (GetOutputStreamMask() & STREAM_MASK_JPEG) {
			if (0/*GetCameraPictureMode() == CAMERA_NORMAL_CONTINUE_SHOT_MODE
				...
				if(FILT_FRM_NUM(m_CapFrmCnt, ZSLCAPFRMINTERVAL)) {
					if(GetZslReprocStreamStatus() && GetOutputStreamMask() & STREAM_MASK_ZSL && 0 == GetReprocessingFlag()){
						...
						****************************************************************************/
						...
						EnqJpgOrZslGraphicBuf((int32_t *)tmpFrm, difftimebak, STREAM_ID_ZSL);
					}
				}
			}
		}
	...
	case CAMERA_EXIT_CB_FAILED:			//Execution failed or rejected
		HAL_LOGE("SprdCameraHardware::camera_cb: @CAMERA_EXIT_CB_FAILURE in state %s.",
...
void SprdCameraHWInterface2::HandleStopCamera(camera_cb_type cb, void *parm4)
{
	HAL_LOGD("in: cb = %d, parm4 = 0x%lx, state = %s",
								  ...
								  void *parm4)
{
	...
	case CAMERA_EVT_CB_FOCUS_MOVE:
		m_notifyCb(CAMERA2_MSG_AUTOFOCUS, parm4 == NULL?(ANDROID_CONTROL_AF_STATE_INACTIVE):(ANDROID_CONTROL_AF_STATE_PASSIVE_SCAN), m_camCtlInfo.afTrigID, (int32_t)parm4, m_callbackClient);
		break;
...
template <typename T>
void SprdCameraHWInterface2::CancelStreamBuf(T *para, bool isCancelAll, uint16_t Stream)
{
	#if 0
	stream_parameters_t     *targetStreamParms = NULL;
	substream_parameters_t  *subParms    = NULL;
	int i =0, res = 0;

	if (Stream == STREAM_ID_PREVIEW) {
		targetStreamParms = &(m_Stream[STREAM_ID_PREVIEW]->m_parameters);
	...
	} else if (Stream == STREAM_ID_ZSL) {
		subParms = &m_subStreams[STREAM_ID_ZSL];
		for (;i < subParms->numSvcBuffers; i++) {
			if ((subParms->svcBufStatus[i] != ON_SERVICE && !isCancelAll) || isCancelAll) {
				res = subParms->streamOps->cancel_buffer(subParms->streamOps, &(subParms->svcBufHandle[i]));
				if (res) {
					HAL_LOGE("cancelbuf res=%d",res);
				}
				subParms->svcBufStatus[i] = ON_HAL_INIT;
			}
		}
	}
	#else
	...
	for (;i < para->numSvcBuffers; i++) {
		if ((para->svcBufStatus[i] != ON_SERVICE && !isCancelAll) || isCancelAll) {
			res = para->streamOps->cancel_buffer(para->streamOps, &(para->svcBufHandle[i]));
			if (res) {
				HAL_LOGE("cancelbuf res=%d",res);
			}
			para->svcBufStatus[i] = ON_HAL_INIT;
		}
	}
	#endif
}
...
template <typename T>
bool SprdCameraHWInterface2::InitStreamBuf(T *stream, int deqNum, int *phySrc, uint32_t *virtSrc,
					int *phyDst, uint32_t *virtDst, int32_t *arryDynamic)
{
	...
	for(i=0; i < deqNum; i++) {
		found = false;
		ret = stream->streamOps->dequeue_buffer(stream->streamOps, &buf);
		if (ret != NO_ERROR || buf == NULL) {
			HAL_LOGD("first dequeue_buffer fail");
			return false;
		}
		for (Index = 0; Index < stream->numSvcBuffers ; Index++) {
			if (*buf == stream->svcBufHandle[Index]
				&& (stream->svcBufStatus[Index] == ON_HAL_INIT || stream->svcBufStatus[Index] == ON_SERVICE)) {
				found = true;
				HAL_LOGD("@@@ Index=%d dymic=%d phy=0x%x",Index, DynamicIndex, phySrc[Index]);
				stream->svcBufStatus[Index] = ON_HAL_DRIVER;
				phyDst[DynamicIndex] = phySrc[Index];
				virtDst[DynamicIndex] = virtSrc[Index];
				arryDynamic[DynamicIndex] = Index;
				stream->bufIsCancel[Index] = false;
				DynamicIndex++;
				break;
			}
		}
		if (!found) {
			HAL_LOGD("error cannot found buf=0x%x",(uint32_t)buf);
			ret = stream->streamOps->cancel_buffer(stream->streamOps, buf);
			if (ret) {
	...
	/*hal do not release these bufs*/
	for (i = 0; i < stream->numSvcBuffers; i++) {
		if (stream->svcBufStatus[i] == ON_SERVICE ||
				stream->svcBufStatus[i] == ON_HAL_INIT) {
			phyDst[DynamicIndex] = phySrc[i];
			virtDst[DynamicIndex] = virtSrc[i];
			arryDynamic[DynamicIndex] = i;
			HAL_LOGD("@@@ Index=%d dymic=%d sta=%d", i, DynamicIndex, stream->svcBufStatus[i]);
			stream->bufIsCancel[i] = true;
			DynamicIndex++;
		}
	}
	/*check buf state*/
	if (DynamicIndex != stream->numSvcBuffers) {
		HAL_LOGD("Error buf stat dymic=%d", DynamicIndex);
		return false;
	}
	return true;
}

bool SprdCameraHWInterface2::getStreamBuffer(bool isPrvAftCap, int streamID, int deqNum)
{
	stream_parameters_t     *targetStreamParms = NULL;
	substream_parameters_t  *subParms    = NULL;
	...
	case STREAM_ID_PREVIEW:
		targetStreamParms = &(m_Stream[STREAM_ID_PREVIEW]->m_parameters);
		return InitStreamBuf(targetStreamParms, deqNum, (int *)mPreviewHeapArray_phy, (uint32_t *)mPreviewHeapArray_vir,
								(int *)mPreviewHeapArray_phy_dynamic, (uint32_t *)mPreviewHeapArray_vir_dynamic,
								(int32_t *)mPreviewBufIndex_dynamic);
		break;
	case STREAM_ID_ZSL:
		subParms = &m_subStreams[STREAM_ID_ZSL];
		if (InitStreamBuf(subParms, deqNum, (int *)subParms->subStreamGraphicFd, (uint32_t *)subParms->subStreamAddVirt,
								(int *)mZslBufArray_phy_dynamic, (uint32_t *)mZslBufArray_vir_dynamic,
								(int32_t *)mZslBufIndex_dynamic)) {
			if (0/*camera_set_zsl_buf_mem((uint32_t)mZslBufArray_phy_dynamic,
						(uint32_t)mZslBufArray_vir_dynamic,
						(subParms->width * subParms->height * 3) / 2,
						(uint32_t)subParms->numSvcBuffers)*/) {
					return false;
			} else {
				return true;
			}
	...
	HAL_LOGE("error strmid=%d", streamID);
	return false;
}
				...
				int num_buffers, buffer_handle_t *registeringBuffers)
{
	...
	if (stream_id == STREAM_ID_PREVIEW && m_Stream[STREAM_ID_PREVIEW] != NULL) {
		...
		for (i = 0; i < targetStreamParms->numSvcBuffers; i++) {
			...
			if (!m_IsNeedHalAllocPrvBuf) {
				mPreviewHeapArray_phy[i] = (cmr_int)phyaddr;
				mPreviewHeapArray_vir[i] = (cmr_int)priv_handle->base;
				mPreviewHeapArray_phy_dynamic[i] = mPreviewHeapArray_phy[i];
				mPreviewHeapArray_vir_dynamic[i] = mPreviewHeapArray_vir[i];
			}

			targetStreamParms->phyAdd[i] = (cmr_int)phyaddr;
			targetStreamParms->svcBufHandle[i] = registeringBuffers[i];//important
			targetStreamParms->svcBufStatus[i] = ON_HAL_INIT;
			targetStreamParms->bufIsRecRelease[i] = true;
			...
			if (m_grallocHal) {
				...
				if(s_mem_method == 0){
					if (targetParms->usage & GRALLOC_USAGE_CAMERA_BUFFER) {
						MemoryHeapIon::Get_phy_addr_from_ion(priv_handle->share_fd,&phyaddr,&size);
						targetParms->subStreamGraphicFd[i] = (cmr_int)phyaddr;
				...
				targetParms->subStreamAddVirt[i]   = (cmr_uint)priv_handle->base;
...
#define FREE_MM_IOVA \
	for (;i < subParms->numSvcBuffers;i++) {\
		if(s_mem_method == 1) {\
			...
			HAL_LOGD("i=%d phy=0x%lx size=0x%x", i, subParms->subStreamGraphicFd[i], subParms->phySize[i]);\
	...
	if (stream_id == STREAM_ID_PREVIEW) {
		...
		}
		{
			...
			for (;i < targetStreamParms->numSvcBuffers;i++) {
				if (s_mem_method == 1) {
					...
					if (m_IsNeedHalAllocPrvBuf)
						MemoryHeapIon::Free_mm_iova(priv_handle->share_fd,mPreviewHeapArray_phy[i], mPreviewHeapArray[i]->phys_size);
					else
			...
			if (iSZslMode()
				...
				|| GetCameraCaptureIntent(&m_staticReqInfo) == CAPTURE_INTENT_VIDEO_RECORD
				#endif
				) {
				freeCaptureMem();
				set_ddr_freq(BASE_FREQ_REQ);
		...
		#endif
		FREE_MM_IOVA
		CancelStreamBuf(subParms, true, STREAM_ID_JPEG);
		...
		SetRecStopMsg(true);
		FREE_MM_IOVA
		CancelStreamBuf(subParms, true, STREAM_ID_RECORD);/*force cancel buf for enq buf to encode, sometime return handle error*/
		...
		FREE_MM_IOVA
		//camera_set_zsl_buf_mem(0, 0, 0, 0);
    ...
    case CAMERA2_TRIGGER_CANCEL_AUTOFOCUS:
		...
		case CAMERA_FOCUS_MODE_INFINITY:
			if (m_focusStat == FOCUS_STAT_ACTIVE_SCAN) {
                if(camera_cancel_autofocus(mCameraHandle))
...
int SprdCameraHWInterface2::getMetadataVendorTagOps(vendor_tag_query_ops_t **ops)
{
    HAL_LOGV("start");
	if (!mVendorTagOps) {
		mVendorTagOps = (vendor_tag_query_ops_t *)malloc(sizeof(vendor_tag_query_ops_t));
		mVendorTagOps->get_camera_vendor_section_name = getVendorSectionName;
		mVendorTagOps->get_camera_vendor_tag_type = getVendorTagType;
		mVendorTagOps->get_camera_vendor_tag_name = getVendorTagName;
		mVendorTagOps->get_camera_vendor_tag_count = getVendorTagCnt;
		mVendorTagOps->get_camera_vendor_tags = getVendorTags;
		*ops = mVendorTagOps;
	}
    return 0;
}
...
int SprdCameraHWInterface2::CameraConvertCropRegion(struct cmr_zoom_param *zoomInfo, struct img_rect *cropRegion)
{
	...
	if(m_CameraId == 0) {
		sensorOrgW = SIZE_ALIGN(jpegResolutionSensorBack[0]);
		sensorOrgH = SIZE_ALIGN(jpegResolutionSensorBack[1]);
	} else if(m_CameraId == 1) {
		sensorOrgW = SIZE_ALIGN(jpegResolutionSensorFront[0]);
		sensorOrgH = SIZE_ALIGN(jpegResolutionSensorFront[1]);
	}
    if (cropRegion->width == 0 || cropRegion->height == 0 || !zoomInfo){
	...
	zoomInfo->zoom_info.zoom_ratio = zoomRatio;
	zoomInfo->zoom_info.output_ratio = minOutputRatio;
	#if 0
	if(IsRotate) {
	...
	#endif
    HAL_LOGD("Crop calculated (ratio=%f)", zoomRatio);
	return ret;
}
...
void SprdCameraHWInterface2::SetCameraZoomRect(struct img_rect *crop)
{
	struct cmr_zoom_param zoom;

	zoom.mode = ZOOM_INFO;
	CameraConvertCropRegion(&zoom, crop);
	SET_PARM(mCameraHandle, CAMERA_PARAM_ZOOM,(cmr_uint)&zoom);
}
...
bool SprdCameraHWInterface2::ProcZslCapReq(int32_t mask)
{
	...
	if (!FindStreamBuf(subParms, virtAdd, (uint32_t *)subParms->subStreamAddVirt, &checkingIndex))
		return false;
	{
		Mutex::Autolock lock(m_capBufMutex);
...
int SprdCameraHWInterface2::CameraPreviewReq(camera_req_info *srcreq,bool *IsSetPara)
{
	int ret = 0;
	status_t res = 0;
	enum camera_param_type drvTag = (enum camera_param_type)0;
	...
	if (m_Stream[STREAM_ID_PREVIEW] != NULL) {
		...
		if (m_Stream[STREAM_ID_PREVIEW]->getHalStopMsg()) {
			...
			if (mCameraState.preview_state == SPRD_IDLE) {
				*IsSetPara = false;
				...
				if (/*CAMERA_ANDROID_ZSL_MODE == GetCameraPictureMode()  //for cts testPreviewPictureSizesCombination with zsl because size changed
					|| */CAMERA_ZSL_MODE == GetCameraPictureMode() || CAMERA_NORMAL_MODE == GetCameraPictureMode()) { //for cts testRecordingHint with zsl
					...
					if (/*CAMERA_ANDROID_ZSL_MODE == GetCameraPictureMode() || */CAMERA_ZSL_MODE == GetCameraPictureMode()) {
						...
						if (0/*CAMERA_ANDROID_ZSL_MODE == GetCameraPictureMode()*/) {
							CancelStreamBuf(zslParms, false,STREAM_ID_ZSL);
							if(!getStreamBuffer(true, STREAM_ID_ZSL, 3)) {
								ret = 3;
								goto preview_req_exit;
							}
				...
				SetPreviewMode(IS_DV_SCENE(m_staticReqInfo));
			if (!m_IsNeedHalAllocPrvBuf) {
				CancelStreamBuf(targetStreamParms, false, STREAM_ID_PREVIEW);
				if (!getStreamBuffer(true, STREAM_ID_PREVIEW, targetStreamParms->numSvcBuffers - targetStreamParms->minUndequedBuffer - 1)) {
					*IsSetPara = true;/*framework recovery when bind died*/
					goto preview_req_exit;
				}
				#if 0
				if (camera_set_preview_mem((uint32_t)mPreviewHeapArray_phy_dynamic,
							(uint32_t)mPreviewHeapArray_vir_dynamic,
							(targetStreamParms->width * targetStreamParms->height * 3)/2,
			...
			cmr_int qret = camera_start_preview(mCameraHandle, GetCameraPictureMode());
			...
			if (mCameraState.preview_state == SPRD_INIT || mCameraState.preview_state == SPRD_IDLE) {
				*IsSetPara = false;
				...
				#endif
				if (!m_IsNeedHalAllocPrvBuf)
					getStreamBuffer(false, STREAM_ID_PREVIEW, targetStreamParms->numSvcBuffers - targetStreamParms->minUndequedBuffer - 1);
						...
						*IsSetPara = false;
						if (!IsCapIntChange) {//for cts testVideoSnapshot start
							...
							if (GetCameraPictureMode() == CAMERA_ZSL_MODE) {
								//SET_PARM(mCameraHandle, CAMERA_PARM_DCDV_MODE, 1);
							}
						}
						#if 0
						if (srcreq->isCropSet) {
						...
						#endif
						if (!m_IsNeedHalAllocPrvBuf)
							getStreamBuffer(true, STREAM_ID_PREVIEW, targetStreamParms->numSvcBuffers - targetStreamParms->minUndequedBuffer - 1);
						startPreviewInternal(false);
					}
				}
		}
...
int SprdCameraHWInterface2::GetJpegOrZslGraphicBuf(uint16_t subStream)
{
	...
	if (!FindStreamBuf(subParms, *buf, (buffer_handle_t *)subParms->svcBufHandle, &Index)) {
		ret = 2;
	...
	case STREAM_ID_JPEG:
		...
		break;
	case STREAM_ID_ZSL:
		if (subParms->bufIsCancel[Index]) {
			subParms->svcBufStatus[Index] = ON_HAL_DRIVER;
			subParms->bufIsCancel[Index] = false;
		} else if (subParms->svcBufStatus[Index] == ON_SERVICE) {
			if (!FindStreamBuf(subParms, Index, (int *)mZslBufIndex_dynamic, &OemIndex)) {
				HAL_LOGE("Error not found index=%d!",Index);
				ret = 1;
				goto cancel_buf;
			}
			//ret = camera_release_capture_frame(subParms->zslCapFrmInfo[Index].channel_id, OemIndex);
			if (ret) {
				HAL_LOGD("error release buf to oem");
			} else {
				subParms->svcBufStatus[Index] = ON_HAL_DRIVER;
			}
		} else {
			HAL_LOGE("Error buf stat=%d",subParms->svcBufStatus[Index]);
			ret = 1;
			goto cancel_buf;
		}
		if (!ret)
...
int SprdCameraHWInterface2::CameraCaptureReq(camera_req_info *srcreq,bool *IsSetPara)
{
	...
	if (mCameraState.capture_state == SPRD_INIT || mCameraState.capture_state == SPRD_IDLE) {
		*IsSetPara = false;
			...
			if(GetCameraPictureMode() == CAMERA_NORMAL_MODE || GetCameraPictureMode() == CAMERA_HDR_MODE
				...
				if(mCameraState.preview_state == SPRD_PREVIEW_IN_PROGRESS) {
					...
					if (m_degenerated_normal_cap) {
						...
						stopPreviewSimple();
						freeCaptureMem();/*for cts testRecordingHint*/
					}
				}
		...
		SET_PARM(mCameraHandle, CAMERA_PARAM_SHOT_NUM, srcreq->capMode);
		if (srcreq->isCropSet) {
			camera_get_sensor_mode_trim(2, &zoom1, &wid, &height);
			if(CameraConvertCropRegion(zoom1.crop_w,zoom1.crop_h,&zoom)) {
				HAL_LOGE("error: scale up over 4times!");
				*IsSetPara = true;
				ret = 2;
				goto capture_req_exit;
			}
			SET_PARM(mCameraHandle, drvTag,(uint32_t)&zoom);
			srcreq->isCropSet = false;
		}
...
void SprdCameraHWInterface2::Camera2ProcessReq( camera_req_info *srcreq)
{
	...
	if (srcreq->isCropSet) {
		...
		zoom = srcreq->cropRegion;
		if (1/*mCameraState.preview_state == SPRD_PREVIEW_IN_PROGRESS
			&& tmpIntent != CAPTURE_INTENT_STILL_CAPTURE*/) {
			if (1/*!((tmpIntent == CAPTURE_INTENT_VIDEO_RECORD
				#ifndef CONFIG_CAMERA_SMALL_PREVSIZE
				|| tmpIntent == CAPTURE_INTENT_PREVIEW
				#endif
				) && IsCapIntChange)*/) {//for cts testVideoSnapshot
				SetCameraZoomRect(&zoom);
				srcreq->isCropSet = false;
			}
		}
...
void SprdCameraHWInterface2::DirectProcessRecReleaseBuf(int bufID, camera_metadata_t *req)
{
	int bufIDs = -1;
	status_t res = 0;
	int32_t outStreamMask = 0;
	stream_parameters_t *targetStreamParms = &(m_Stream[STREAM_ID_PREVIEW]->m_parameters);
	...
	targetStreamParms->bufIsRecRelease[(bufIDs >> 8) & 0xff] = true;
	bufIDs &= 0xff;
	if (bufIDs  == 0xff || !(bufIDs >= 0 && bufIDs < targetStreamParms->numSvcBuffers)) {
		goto free_req;
	}
	HAL_LOGV("release=%d,stat=%d", targetStreamParms->bufIsRecRelease[bufIDs],
			targetStreamParms->svcBufStatus[mPreviewBufIndex_dynamic[bufIDs]]);
	if (targetStreamParms->bufIsRecRelease[bufIDs]
		&& targetStreamParms->svcBufStatus[mPreviewBufIndex_dynamic[bufIDs]] != ON_HAL_DRIVER) {
		res = camera_release_frame(mCameraHandle, CAMERA_VIDEO_DATA, bufIDs);//camera_release_frame(bufIDs);
		targetStreamParms->svcBufStatus[mPreviewBufIndex_dynamic[bufIDs]] = ON_HAL_DRIVER;
	} else {
		HAL_LOGD("Error buf stat");
	}
free_req:
	res = m_requestQueueOps->free_request(m_requestQueueOps, req);
	if (res < 0)
		HAL_LOGE("ERR: free_request res = %d", res);
}

void SprdCameraHWInterface2::Camera2GetSrvReqInfo( camera_req_info *srcreq, camera_metadata_t *orireq, bool *isDirectProc)
{
	status_t res = 0;
	camera_metadata_entry_t entry;
	uint32_t reqCount = 0;
	...
	*isDirectProc = false;
        ...
        if (get_camera_metadata_entry(srcreq->ori_req, index, &entry)==0) {
			...
			case ANDROID_LENS_FOCAL_LENGTH:
				if (0 != entry.count) {
					...
					SET_PARM(mCameraHandle, CAMERA_PARAM_FOCAL_LENGTH,  (int32_t)(entry.data.f[0]*1000));
				}
				break;
			case ANDROID_JPEG_QUALITY:
				if (0 != entry.count) {
					...
					SET_PARM(mCameraHandle, CAMERA_PARAM_JPEG_QUALITY, jpeg_q);
				}
				break;
			case ANDROID_JPEG_THUMBNAIL_QUALITY:
				if (0 != entry.count) {
					...
					SET_PARM(mCameraHandle, CAMERA_PARAM_THUMB_QUALITY, th_q);
				}
				break;
            ...
            case ANDROID_REQUEST_TYPE:{
					Mutex::Autolock lock(m_halCBMutex);
					ASIGNIFNOTEQUAL(srcreq->isReprocessing, entry.data.u8[0], (enum camera_param_type)NULL)
			...
			case ANDROID_JPEG_GPS_TIMESTAMP:
				ASIGNIFNOTEQUAL(srcreq->gpsTimestamp, entry.data.i64[0], (enum camera_param_type)NULL)
				break;
			...
			case ANDROID_JPEG_THUMBNAIL_SIZE:
				...
				jpeg_thumb_size.width = srcreq->thumbnailJpgSize.width;
				jpeg_thumb_size.height = srcreq->thumbnailJpgSize.height;
			...
			case ANDROID_JPEG_ORIENTATION:
			{
				...
				SET_PARM(mCameraHandle, CAMERA_PARAM_ENCODE_ROTATION, jpeg_orientation);
			}
				break;
			case ANDROID_CONTROL_MODE:
				ASIGNIFNOTEQUAL(srcreq->ctlMode, (ctl_mode)(entry.data.u8[0] + 1),(enum camera_param_type)NULL)
			...
			case ANDROID_SCALER_CROP_REGION:
				zoom0.start_x= entry.data.i32[0] & ALIGN_ZOOM_CROP_BITS;
				zoom0.start_y= entry.data.i32[1] & ALIGN_ZOOM_CROP_BITS;
				zoom0.width = entry.data.i32[2] & ALIGN_ZOOM_CROP_BITS;
				zoom0.height = entry.data.i32[3] & ALIGN_ZOOM_CROP_BITS;
				if (zoom0.start_x!= srcreq->cropRegion.start_x || zoom0.start_y != srcreq->cropRegion.start_y\
					  || zoom0.width!= srcreq->cropRegion.width || zoom0.height != srcreq->cropRegion.height) {
					srcreq->cropRegion = zoom0;
					srcreq->isCropSet = true;
				}
				HAL_LOGD("ANDROID_SCALER_CROP_REGION (%d %d %d %d)",
						zoom0.start_x,zoom0.start_y,zoom0.width,zoom0.height);
				break;
		    case ANDROID_CONTROL_AF_MODE:
				{
					...
					if (m_CameraId == 0) {
						if (srcreq->afMode != AfMode) {
							...
							SET_PARM(mCameraHandle, drvTag,AfMode);
						}
					}
				}
                break;
			...
			case ANDROID_SPRD_CAPTURE_MODE:
				{
					...
					ASIGNIFNOTEQUAL(srcreq->capMode, capMode, (enum camera_param_type)NULL)
			...
			case ANDROID_SPRD_BURST_CAP_CNT:
				{
					...
					if (capCnt < srcreq->capMode && capCnt > 0 && m_camCtlInfo.burstCapCnt > 0) {/*burst cap sync signal from framework*/
						ASIGNIFNOTEQUAL(m_camCtlInfo.burstCapSync, true, (enum camera_param_type)NULL)
            ...
            case ANDROID_CONTROL_AF_REGIONS:
				{
					...
					SET_PARM(mCameraHandle, CAMERA_PARAM_FOCUS_RECT, (int32_t)area);
				}
                break;
			...
			case ANDROID_CONTROL_AE_MODE:
                {
					...
					if (m_CameraId == 0) {
						if (srcreq->flashMode != FlashMode) {
							...
							SET_PARM(mCameraHandle, drvTag,FlashMode);
						}
					}
				}
                break;
			case ANDROID_CONTROL_AE_LOCK:
				ASIGNIFNOTEQUAL(srcreq->aeLock, (ae_lock)entry.data.u8[0],(enum camera_param_type)NULL)
                break;
            ...
            case ANDROID_CONTROL_AWB_LOCK:
                ASIGNIFNOTEQUAL(srcreq->awbLock, (awb_lock)entry.data.u8[0],(enum camera_param_type)NULL)
                break;
            case ANDROID_REQUEST_ID:
				ASIGNIFNOTEQUAL(srcreq->requestID, entry.data.i32[0],(enum camera_param_type)NULL)
                HAL_LOGD("ANDROID_REQUEST_ID (%d)", entry.data.i32[0]);
                break;
            case ANDROID_REQUEST_METADATA_MODE:
				ASIGNIFNOTEQUAL(srcreq->metadataMode, (metadata_mode)entry.data.u8[0],(enum camera_param_type)NULL)
            ...
            case ANDROID_REQUEST_FRAME_COUNT: {
				Mutex::Autolock lock(m_halCBMutex);
					ASIGNIFNOTEQUAL(srcreq->frmCnt,entry.data.i32[0],(enum camera_param_type)NULL)
	...
	if (CAPTURE_INTENT_VIDEO_RECORD == srcreq->captureIntent
		...
		#endif
		//isRecord = true;
		if (srcreq->prvFrmRate != frameRate) {
			srcreq->prvFrmRate = frameRate;
		}
	} else {
		if (srcreq->sceneMode == CAMERA_SCENE_MODE_HDR) {
			//picMode = CAMERA_HDR_MODE;
		}
	}
	//SET_PARM(mCameraHandle, CAMERA_PARM_DCDV_MODE, isRecord);
...
void SprdCameraHWInterface2::SetDDRFreqBefStartPrv(void)
{
	stream_parameters_t     *targetStreamParms = NULL;
	...
	if (CAMERA_ZSL_MODE == GetCameraPictureMode()
		/*|| CAMERA_ANDROID_ZSL_MODE == GetCameraPictureMode()*/) {
			set_ddr_freq(HIGH_FREQ_REQ);
	} else {
		#ifdef CONFIG_CAMERA_SMALL_PREVSIZE
		if (GetCameraCaptureIntent(&m_staticReqInfo) == CAPTURE_INTENT_VIDEO_RECORD)
			set_ddr_freq(MEDIUM_FREQ_REQ);
		else
		#else
		if (targetStreamParms->height == 1088 && GetOutputStreamMask() & STREAM_MASK_PRVCB)
			set_ddr_freq(MEDIUM_FREQ_REQ);
		else
		#endif
			set_ddr_freq(BASE_FREQ_REQ);
...
int SprdCameraHWInterface2::Callback_Free(enum camera_mem_cb_type type, cmr_uint *phy_addr, cmr_uint *vir_addr, cmr_u32 sum, void* private_data)
{
	SprdCameraHWInterface2 *camera = (SprdCameraHWInterface2 *)private_data;
	int                 ret = 0;
	...
	if (CAMERA_PREVIEW == type) {
		if (camera->m_IsNeedHalAllocPrvBuf) {
			camera->freePreviewMem(sum);
		} else {
			if (camera->m_Stream[STREAM_ID_PREVIEW] != NULL)
				HAL_LOGD("preview stream not release");
		}
	} else {
...
int SprdCameraHWInterface2::Callback_Malloc(enum camera_mem_cb_type type, cmr_u32 *size, cmr_u32 *sum, cmr_uint *phy_addr,
												cmr_uint *vir_addr, void* private_data)
{
	SprdCameraHWInterface2 *camera = (SprdCameraHWInterface2 *)private_data;
	int                 ret = 0;
	substream_parameters_t  *targetParms = &camera->m_subStreams[STREAM_ID_ZSL];
	stream_parameters_t     *targetStreamParms = NULL;
	int i = 0;

	if (!private_data || !phy_addr || !vir_addr || !size || !sum) {
		HAL_LOGE("error param 0x%lx 0x%lx 0x%lx", (cmr_uint)phy_addr, (cmr_uint)vir_addr, (cmr_uint)private_data);
		return -1;
	}
	if (type >= CAMERA_MEM_CB_TYPE_MAX) {
	...
	if (CAMERA_PREVIEW == type) {
		if (camera->m_IsNeedHalAllocPrvBuf) {
			ret = camera->allocatePreviewMem(size, sum, phy_addr, vir_addr);
		} else {
			if (camera->m_Stream[STREAM_ID_PREVIEW] == NULL) {
				HAL_LOGE("preview stream is null");
				return -1;
			} else {
				targetStreamParms = &(camera->m_Stream[STREAM_ID_PREVIEW]->m_parameters);
				*size = (targetStreamParms->width * targetStreamParms->height * 3)/ 2;
				*sum = targetStreamParms->numSvcBuffers;
				for (; i < targetStreamParms->numSvcBuffers; i++) {
					phy_addr[i] = (cmr_uint)camera->mPreviewHeapArray_phy_dynamic[i];
					vir_addr[i] = (cmr_uint)camera->mPreviewHeapArray_vir_dynamic[i];
				}
			}
		}
	} else {
...
void SprdCameraHWInterface2::SetPreviewMode(bool isDVMode)
{
	struct cmr_preview_fps_param fps_param;

	fps_param.is_recording = GetOutputStreamMask() & STREAM_MASK_RECORD;
	if (isDVMode) {
		fps_param.frame_rate = (cmr_uint)m_staticReqInfo.prvFrmRate;
...
status_t SprdCameraHWInterface2::startPreviewInternal(bool isChangPrvSize)
{
	stream_parameters_t     *targetStreamParms = NULL;
	bool  ret = true;
	uint32_t memSize = 0;
	...
	HAL_LOGV("start preview changsize=%d.",isChangPrvSize);
	if (isPreviewing()) {
...
/*	setRecordingMode(isRecording);*/
	...
	androidPreviewFmtToDrvPrvFmt(targetStreamParms->format, &format);
	SET_PARM(mCameraHandle, CAMERA_PARAM_PREVIEW_FORMAT, (cmr_uint)format);
	#if 0
	if (IS_DV_SCENE(m_staticReqInfo)) {
		memSize = (SIZE_ALIGN(targetStreamParms->width) * SIZE_ALIGN(targetStreamParms->height) * 3) / 2;
	} else {
		memSize = (targetStreamParms->width * targetStreamParms->height * 3) / 2;
	}
	if (m_IsNeedHalAllocPrvBuf) {
		if (camera_set_preview_mem((uint32_t)mPreviewHeapArray_phy,
				(uint32_t)mPreviewHeapArray_vir,
	...
	#endif
	if (camera_set_mem_func(mCameraHandle, (void*)Callback_Malloc, (void*)Callback_Free, (void*)this)) {
		return SPRD_ERROR;
	}
	if (CAMERA_ZSL_MODE == GetCameraPictureMode()
		/*|| CAMERA_ANDROID_ZSL_MODE == GetCameraPictureMode()*/) {
	...
	SetDDRFreqBefStartPrv();
	setCameraState(SPRD_INTERNAL_PREVIEW_REQUESTED, STATE_PREVIEW);
	cmr_int qret = camera_start_preview(mCameraHandle, GetCameraPictureMode());//mode
...
void SprdCameraHWInterface2::stopPreviewInternal(bool isCancelAllBuf)
{
	...
	if (targetStreamParms != NULL) {
		CancelStreamBuf(targetStreamParms, isCancelAllBuf, STREAM_ID_PREVIEW);
		if (isCancelAllBuf)
...
int SprdCameraHWInterface2::displaySubStream(sp<Stream> stream, int32_t *srcBufVirt, int64_t frameTimeStamp, uint16_t subStream)
{
	...
	case STREAM_ID_PRVCB:
		if (subParms->format == HAL_PIXEL_FORMAT_YCrCb_420_SP) {
			memcpy((char *)(priv_handle->base), srcBufVirt, (subParms->width * subParms->height * 3) / 2);
		} else if (subParms->format == HAL_PIXEL_FORMAT_YV12) {
			register uint32_t i = 0;
			register uint32_t j = 0;
			register uint32_t wid = subParms->width;
			register uint32_t height = subParms->height;
			register char *dataSrc = (char *)srcBufVirt;
			register char *dataDst = (char *)(priv_handle->base);
			register char *dataDstV = NULL;
			/*width/2 is not align 16, discard some pixels*/
			memcpy(dataDst, dataSrc, wid * height);
			dataSrc = (char *)srcBufVirt + wid * height;
			dataDst = (char *)(priv_handle->base) + SIZE_ALIGN(wid) * height;
			dataDstV = dataDst + ((SIZE_ALIGN(wid >> 1) * height) >> 1);
			for (; i < (height >> 1); i++) {
				for (j = 0; j < (wid >> 1); j++) {
					*dataDst++ = *dataSrc++;
					*dataDstV++ = *dataSrc++;
			...
			HAL_LOGE("error not support format=%d",subParms->format);
			goto cancel_buf;
		}
		break;
...
int SprdCameraHWInterface2::EnqJpgOrZslGraphicBuf(void *srcBufVirt, int64_t frameTimeStamp, uint16_t subStream)
{
	#if 0
	substream_parameters_t  *subParms    = &m_subStreams[subStream];
	cmr_uint                    VirtBuf = 0;
	...
	} else if (subStream == STREAM_ID_ZSL) {
		if (subParms->format != HAL_PIXEL_FORMAT_YCrCb_420_SP) {
			HAL_LOGE("error zsl format=0x%x", subParms->format);
			return 1;
		} else {
			VirtBuf = (uint32_t)(((camera_cap_frm_info *)srcBufVirt)->buf_Virt_Addr);
			if (!FindStreamBuf(subParms, VirtBuf, (uint32_t *)subParms->subStreamAddVirt, &Index)) {
				HAL_LOGE("ERR cannot found buf=0x%x", VirtBuf);
				return 1;
			} else {
				subParms->zslCapFrmInfo[Index] = ((camera_cap_frm_info *)srcBufVirt)->cap_info;
			}
	...
	case STREAM_ID_ZSL:
		{
			...
			{
				Mutex::Autolock lock(m_capBufMutex);
				if(subParms->svcBufStatus[Index] != ON_HAL_DRIVER && subParms->svcBufStatus[Index] != ON_HAL_INIT) {
					HAL_LOGE("Error buf stat=%d", subParms->svcBufStatus[Index]);
					subParms->svcBufStatus[Index] = ON_HAL_BUFERR;
					return 1;
				}
				if (subParms->svcBufStatus[Index] == ON_HAL_DRIVER) {
					m_zslValidDataSize.width = cap_width  = ((camera_cap_frm_info *)srcBufVirt)->width;
					m_zslValidDataSize.height = cap_height = ((camera_cap_frm_info *)srcBufVirt)->height;
					...
					subParms->svcBufStatus[Index] = ON_SERVICE;//have new frm_info
				} else if (subParms->svcBufStatus[Index] == ON_HAL_INIT) {
					if (!FindStreamBuf(subParms, Index, (int *)mZslBufIndex_dynamic, &OemIndex)) {
						HAL_LOGE("Error not found index=%d!",Index);
						return 1;
					}
...
void SprdCameraHWInterface2::HandleStartCamera(camera_cb_type cb,
									void *parm4)
{
	HAL_LOGV("in: cb = %d, parm4 = 0x%lx, state = %s",
				cb, (cmr_uint)parm4, getCameraStateStr(getCameraState()));
...
void SprdCameraHWInterface2::HandleStartPreview(enum camera_cb_type cb,
											 void *parm4)
{
	...
	switch(cb) {
	case CAMERA_RSP_CB_SUCCESS:
		if (!(getPreviewState() == SPRD_PREVIEW_IN_PROGRESS && (m_staticReqInfo.isCropSet == true ||
			GetCameraCaptureIntent(&m_staticReqInfo) == CAPTURE_INTENT_VIDEO_RECORD))) {/*zoom maybe fail, but hal preview status not be changed*/
	...
	case CAMERA_EXIT_CB_FAILED:
		HAL_LOGE("camera_cb: @CAMERA_EXIT_CB_FAILURE in state %s.",
	...
	case CAMERA_EVT_CB_FLUSH:
		{
			...
			if (!frame || m_Stream[STREAM_ID_PREVIEW] == NULL) {
				HAL_LOGD("Err para=0x%lx!", (cmr_int)frame);
				return;
			}
			targetStreamParms = &(m_Stream[STREAM_ID_PREVIEW]->m_parameters);
...
bool SprdCameraHWInterface2::prvFrmCbCheckPrvIsProc(camera_frame_type *frame)
{
	...
	if (!GetIsOutputStream()) {
		res = camera_release_frame(mCameraHandle, CAMERA_PREVIEW_DATA, frame->buf_id);
...
void SprdCameraHWInterface2::receivePreviewFrame(camera_frame_type *frame)
{
	...
	TrueID = mPreviewBufIndex_dynamic[targetStreamParms->bufIndex];
	streamOutMask = GetOutputStreamMask();
	{
		Mutex::Autolock lock(m_halCBMutex);
	...
	HAL_LOGD("@@@ Index=%d trueID=%d status=%d StreamMask=0x%x time=%lld",targetStreamParms->bufIndex,
		TrueID,targetStreamParms->svcBufStatus[TrueID], streamOutMask, targetStreamParms->m_timestamp);
	if (GetStartPreviewAftPic()) {
		SetStartPreviewAftPic(false);
		if (StreamSP->m_IsFirstFrm) {
			StreamSP->m_IsFirstFrm = false;
	...
	//check buf status
	if (targetStreamParms->svcBufStatus[TrueID] != ON_HAL_DRIVER
		&& targetStreamParms->svcBufStatus[TrueID] != ON_HAL_INIT
		&& targetStreamParms->svcBufStatus[TrueID] != ON_SERVICE
		) {
		HAL_LOGE("Error buf stat=(%d)",
		targetStreamParms->svcBufStatus[TrueID]);
		if (!StreamSP->m_IsFirstFrm) {
			StreamSP->m_IsFirstFrm = true;
			m_RequestQueueThread->SetSignal(SIGNAL_REQ_THREAD_REQ_DONE);
		}
	...
	if (StreamSP->getHalStopMsg()) {
		...
		if (targetStreamParms->svcBufStatus[TrueID] != ON_SERVICE) {
			if (targetStreamParms->streamOps->cancel_buffer(targetStreamParms->streamOps, &(targetStreamParms->svcBufHandle[TrueID]))) {
				HAL_LOGE("Error cancelbuf!");
			}
			targetStreamParms->svcBufStatus[TrueID] = ON_HAL_INIT;
		}
	...
	if (streamOutMask & STREAM_MASK_PRVCB) {
		substream_parameters_t *subParameters = &m_subStreams[STREAM_ID_PRVCB];

		if (SUBSTREAM_TYPE_PRVCB == subParameters->type &&
			(subParameters->format == HAL_PIXEL_FORMAT_YCrCb_420_SP || subParameters->format == HAL_PIXEL_FORMAT_YV12)) {
				displaySubStream(StreamSP, (int32_t *)(mPreviewHeapArray_vir[TrueID]),\
							targetStreamParms->m_timestamp,STREAM_ID_PRVCB);
		}
	}

	if (streamOutMask & STREAM_MASK_PREVIEW) {
		buf = &(targetStreamParms->svcBufHandle[TrueID]);
		priv_handle = reinterpret_cast<const private_handle_t *>(*buf);
		phyaddr = targetStreamParms->phyAdd[TrueID];
		size = (targetStreamParms->width * targetStreamParms->height * 3) / 2;
		...
		HAL_LOGD("@@@ Display Preview add=0x%x,enqueue index=%d frmcnt=%d",
			 (uint32_t)targetStreamParms->svcBufHandle[TrueID],TrueID, m_PrvFrmCnt);
		if (targetStreamParms->svcBufStatus[TrueID] == ON_HAL_DRIVER) {
			/*deq one buf*/
			res = targetStreamParms->streamOps->dequeue_buffer(targetStreamParms->streamOps, &buf);
			if (res != NO_ERROR || buf == NULL) {
			...
			if (!FindStreamBuf(targetStreamParms, Index, (int *)mPreviewBufIndex_dynamic, &OemIndex)) {
				if (targetStreamParms->streamOps->cancel_buffer(targetStreamParms->streamOps, buf)) {
					HAL_LOGE("Error cancelbuf!");
				}
				goto proc_req_over;
			}
			res = camera_release_frame(mCameraHandle, CAMERA_PREVIEW_DATA, OemIndex);//camera_release_frame(OemIndex);
			if (res) {
			...
			res = targetStreamParms->streamOps->enqueue_buffer(targetStreamParms->streamOps,
						...
						&(targetStreamParms->svcBufHandle[TrueID]));
			if (res) {
				HAL_LOGD("enqueue fail, ret=%d,buf status=%d.",res, targetStreamParms->svcBufStatus[TrueID]);
				if (targetStreamParms->streamOps->cancel_buffer(targetStreamParms->streamOps, &(targetStreamParms->svcBufHandle[TrueID]))) {
					HAL_LOGE("Error cancelbuf!");
				}
				goto proc_req_over;
			...
			if(0/*CAMERA_ANDROID_ZSL_MODE == GetCameraPictureMode()*/){
				if (FILT_FRM_NUM(m_PrvFrmCnt, ZSLPRVFRMINTERVAL)) {
					if(streamOutMask & STREAM_MASK_ZSL && (0 == GetReprocessingFlag())){
						{
							Mutex::Autolock lock(m_afTrigLock);
			...
			if (targetStreamParms->svcBufStatus[TrueID] == ON_HAL_INIT) {/*cancel buf*/
				/* must not change buf status*/
				HAL_LOGD("not equal srv bufq, index=%d",TrueID);
				res = camera_release_frame(mCameraHandle, CAMERA_PREVIEW_DATA, targetStreamParms->bufIndex);
				if (res) {
					HAL_LOGD("error release buf to oem");
				}
			} else {
				HAL_LOGE("Error buf stat=%d", targetStreamParms->svcBufStatus[TrueID]);
			}
		}
    } else {
		if (!StreamSP->m_IsFirstFrm) {
		...
		HAL_LOGD("stream not output,Index=%d",TrueID);
		res = camera_release_frame(mCameraHandle, CAMERA_PREVIEW_DATA, targetStreamParms->bufIndex);
		if (res) {
...
void SprdCameraHWInterface2::receivePrevFrmWithCacheMem(camera_frame_type *frame)
{
    ...
    if (GetOutputStreamMask() & STREAM_MASK_PRVCB) {
        ...
        for (; i < NUM_MAX_SUBSTREAM ; i++) {
            if (StreamSP->m_attachedSubStreams[i].streamId == STREAM_ID_PRVCB) {
				if (1/*camera_get_rot_set()*/) {
					displaySubStream(StreamSP, (int32_t *)(frame->y_vir_addr),\
						      targetStreamParms->m_timestamp,STREAM_ID_PRVCB);
				}/* else {
    ...
    if (GetOutputStreamMask() & STREAM_MASK_PREVIEW) {
		...
		priv_handle = reinterpret_cast<const private_handle_t *>(*buf);
		if (1/*camera_get_rot_set()*/) {
			memcpy((char *)(priv_handle->base),(char *)(frame->y_vir_addr),
							(targetStreamParms->width * targetStreamParms->height * 3)/2);
		}/* else {
		...
		res = camera_release_frame(mCameraHandle, CAMERA_PREVIEW_DATA, targetStreamParms->bufIndex);//(targetStreamParms->bufIndex);
		if (res) {
             HAL_LOGD("ERR release frame!");
		}
		if (!StreamSP->m_IsFirstFrm) {
    ...
    if (currentSignal & SIGNAL_REQ_THREAD_REQ_Q_NOT_EMPTY) {
		...
		if(!GetReqProcessStatus()){
	        ...
	        else
			{
			    ...
			    Camera2GetSrvReqInfo(&m_staticReqInfo, curReq, &isDirectProc);
				if (!isDirectProc)
...
/*    static const float lensShadingMap[3 * 1 * 1 ] =
   ...
   /* ADD_OR_SIZE(ANDROID_SENSOR_INFO_SENSITIVITY_RANGE,
    ...
    static const int32_t availableTargetFpsRangesBack[] = {
            10, 20, 10, 25, 10, 30 /*4, 25, 5, 30*/
    };

    static const int32_t availableTargetFpsRangesFront[] = {
            10, 15, 10, 20, 10, 25, 10, 30 /*4, 25, 5, 30*/
    };
    if (cameraId == 1) {
        ADD_OR_SIZE(ANDROID_CONTROL_AE_AVAILABLE_TARGET_FPS_RANGES,
                availableTargetFpsRangesFront,
                sizeof(availableTargetFpsRangesFront)/sizeof(int32_t));
    } else {
        ADD_OR_SIZE(ANDROID_CONTROL_AE_AVAILABLE_TARGET_FPS_RANGES,
                availableTargetFpsRangesBack,
                sizeof(availableTargetFpsRangesBack)/sizeof(int32_t));
    }
	...
	#else
	static const uint8_t videoSnapshotSupport = ANDROID_SPRD_VIDEO_SNAPSHOT_SUPPORT_ON;
	#endif
	ADD_OR_SIZE(ANDROID_SPRD_VIDEO_SNAPSHOT_SUPPORT, &videoSnapshotSupport, 1);

	static const uint8_t prvRecDifSize = 0;
	ADD_OR_SIZE(ANDROID_SPRD_PRV_REC_DIFFERENT_SIZE_SUPPORT, &prvRecDifSize, 1);
