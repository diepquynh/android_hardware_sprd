...
/*			if (camera_get_rot_set()) {
		...
		if (isRecordingMode()) {
			...
			if (0 == mGraphBufferCount[index]) {
				if (CMR_CAMERA_SUCCESS != camera_release_frame(mCameraHandle, CAMERA_VIDEO_DATA, index)) {
					goto EXIT;
				}
			}
		}
...
{
	struct cmr_msg message = {0, 0, 0, 0, 0, 0};
	status_t ret = NO_ERROR;
	Mutex::Autolock l(&mLock);
	mParamLock.lock();
...
status_t SprdCameraHardware::setParametersInternal(const SprdCameraParameters& params)
{
	...
	camera_is_change_size(mCameraHandle, mRawWidth, mRawHeight, mPreviewWidth, mPreviewHeight, &is_change);
	if (is_change) {
		if (isPreviewing()) {
			mPreviewLock.lock();
...
bool SprdCameraHardware::allocatePreviewMemFromGraphics(cmr_u32 size, cmr_u32 sum, cmr_uint *phy_addr, cmr_uint *vir_addr)
{
	...
	if (PREVIEW_BUFFER_USAGE_GRAPHICS == mPreviewBufferUsage) {
		...
		for (i = 0; i < (int)sum; i++ ) {
			...
			LOGI("get buffer handle 0x%x", (cmr_u32)private_h);
			if (NULL == private_h) {
			...
			if (0  == s_mem_method) {
				...
				mPreviewHeapArray_phy[i] = (cmr_u32)ion_addr;
				mPreviewHeapArray_vir[i] = (cmr_u32)private_h->base;
				...
				*vir_addr++ = (cmr_uint)private_h->base;
				...
				mPreviewBufferHandle[i] = buffer_handle;
				mPreviewHeapArray_phy[i] = (cmr_u32)iova_addr;
				mPreviewHeapArray_vir[i] = (cmr_u32)private_h->base;
...
void SprdCameraHardware::receivePreviewFrame(struct camera_frame_type *frame)
{
	...
	if(mData_cb != NULL)
	{
		LOGI("receivePreviewFrame mMsgEnabled: 0x%x",mMsgEnabled);
...
void SprdCameraHardware::HandleFocus(enum camera_cb_type cb, void* parm4)
{
	...
	case CAMERA_EVT_CB_FOCUS_MOVE:
		...
		mNotify_cb(CAMERA_MSG_FOCUS_MOVE, (int32_t)parm4, 0, mUser);
		break;
