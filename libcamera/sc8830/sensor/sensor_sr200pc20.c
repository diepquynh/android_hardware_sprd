/******************************************************************************
 ** Copyright (c)
 ** File Name:        sensor_SR200PC20.c                                           *
 ** Author:                                                       *
 ** DATE:                                                              *
 ** Description:   This file contains driver for sensor SR200PC20.
 **
 ******************************************************************************
...
//#include "sensor_sr200pc20_mipi_d98x.h"
//#include "sensor_sr200pc20_mipi_w300.h"
/*lint -save -e765 */
/**--------------------------------------------------------------------------*
 **                         Compiler Flag                                      *
 **---------------------------------------------------------------------------*/
 ...
 **                            Macro Define
 **---------------------------------------------------------------------------*/
#define SR200PC20_I2C_ADDR_W        0x20//0x20
#define SR200PC20_I2C_ADDR_R         0x20//0x20
#define I2C_WRITE_BURST_LENGTH    512

static uint32_t  g_flash_mode_en = 0;
static uint32_t  setmode =0;
static EXIF_SPEC_PIC_TAKING_COND_T s_SR200PC20_exif;

/**---------------------------------------------------------------------------*
 **                     Local Function Prototypes                              *
 **---------------------------------------------------------------------------*/
LOCAL uint32_t _SR200PC20_InitExifInfo(void);
//LOCAL uint32_t _SR200PC20_GetResolutionTrimTab(uint32_t param);
LOCAL uint32_t _SR200PC20_PowerOn(uint32_t power_on);
LOCAL uint32_t _SR200PC20_Identify(uint32_t param);
LOCAL uint32_t _SR200PC20_set_brightness(uint32_t level);
LOCAL uint32_t _SR200PC20_set_saturation(uint32_t level);
LOCAL uint32_t _SR200PC20_set_image_effect(uint32_t effect_type);
LOCAL uint32_t _SR200PC20_set_anti_flicker(uint32_t mode);
LOCAL uint32_t _SR200PC20_set_video_mode(uint32_t mode);
LOCAL uint32_t _SR200PC20_set_awb(uint32_t mode);
LOCAL uint32_t _SR200PC20_set_work_mode(uint32_t mode);
LOCAL uint32_t _SR200PC20_BeforeSnapshot(uint32_t param);
LOCAL uint32_t _SR200PC20_check_image_format_support(uint32_t param);
LOCAL uint32_t _SR200PC20_pick_out_jpeg_stream(uint32_t param);
LOCAL uint32_t _SR200PC20_after_snapshot(uint32_t param);
//LOCAL uint32_t _ov540_flash(uint32_t param);
LOCAL uint32_t _SR200PC20_GetExifInfo(uint32_t param);
//LOCAL uint32_t _SR200PC20_ExtFunc(uint32_t ctl_param);
LOCAL uint32_t SR200PC20_InitExt(uint32_t param);
LOCAL uint32_t sensor_InitTflash(uint32_t param);
LOCAL uint32_t _sr200pc20_StreamOn(uint32_t param);
LOCAL uint32_t _sr200pc20_StreamOff(uint32_t param);
static uint32_t _SR200PC20_GetResolutionTrimTab(uint32_t param);


LOCAL const SENSOR_REG_T SR200PC20_common[]=
{
{0x01, 0x31,}, /* sleep on */
{0x01, 0x33,}, /* sleep on */
{0x01, 0x31,}, /* sleep on */
...
LOCAL const SENSOR_REG_T SR200PC20_640X480[]=
{
{0x03, 0x00,}, /* Sleep On */
{0x01, 0x31,},
...
LOCAL const SENSOR_REG_T SR200PC20_800X600[]=
{
{0x03, 0x00,}, /* Sleep On */
{0x01, 0x31,},
...
LOCAL const SENSOR_REG_T SR200PC20_1600X1200[] =
{
{0x03, 0x00,},
...
LOCAL SENSOR_REG_TAB_INFO_T s_SR200PC20_resolution_Tab_YUV[]=
{
	// COMMON INIT
	{ADDR_AND_LEN_OF_ARRAY(SR200PC20_common),   0, 0, 24, SENSOR_IMAGE_FORMAT_YUV422},
	//{PNULL, 0, 0, 24, SENSOR_IMAGE_FORMAT_YUV422},
	{ADDR_AND_LEN_OF_ARRAY(SR200PC20_640X480),   640,  480,   24, SENSOR_IMAGE_FORMAT_YUV422},
	{ADDR_AND_LEN_OF_ARRAY(SR200PC20_1600X1200), 1600, 1200,  24, SENSOR_IMAGE_FORMAT_YUV422},
	//{PNULL, 0, 0, 0, 0, 0},
	{PNULL, 0, 0, 0, 0, 0},
	{PNULL, 0, 0, 0, 0, 0},
...
static SENSOR_TRIM_T s_SR200PC20_Resolution_Trim_Tab[] = {
...
LOCAL uint32_t SR200PC20_InitExt(uint32_t param)	//wujinyou, 2012.11.14
{
	...
	SENSOR_PRINT("SR200PC20_InitExt start ;%d;\n",param);
	...
	sensor_reg_tab_info_ptr = &s_SR200PC20_resolution_Tab_YUV[param];
	...
	SENSOR_PRINT("SENSOR: SR200PC20_InitExt end, ret=%d, time=%d us\n", ret, (uint32_t)((timestamp_new-timestamp_old)/1000));

	return SENSOR_SUCCESS;
};

LOCAL SENSOR_IOCTL_FUNC_TAB_T s_SR200PC20_ioctl_func_tab =
{
	// Internal
	PNULL,
	PNULL,//_SR200PC20_PowerOn,
	PNULL,
	_SR200PC20_Identify,

	PNULL,            // write register
	PNULL,            // read  register
	PNULL,     //SR200PC20_InitExt,
	_SR200PC20_GetResolutionTrimTab,
	//PNULL,
	...
	_SR200PC20_set_brightness,
	PNULL,//_SR200PC20_set_contrast,
	PNULL,
	_SR200PC20_set_saturation,

	_SR200PC20_set_work_mode,
	_SR200PC20_set_image_effect,

	_SR200PC20_BeforeSnapshot,
	_SR200PC20_after_snapshot,
	PNULL,
	PNULL,
	PNULL,
	PNULL,
	...
	_SR200PC20_set_awb,
	PNULL,
	PNULL,//_SR200PC20_set_iso,
	PNULL,//_SR200PC20_set_ev,
	_SR200PC20_check_image_format_support,
	PNULL,
	PNULL,
	_SR200PC20_GetExifInfo,
	PNULL,//_SR200PC20_ExtFunc,
	_SR200PC20_set_anti_flicker,
	_SR200PC20_set_video_mode,
	_SR200PC20_pick_out_jpeg_stream,
	PNULL, //meter_mode
	PNULL, //get_status
	_sr200pc20_StreamOn,
	_sr200pc20_StreamOff,
	PNULL
};
 ...
 **                         Global Variables                                  *
 **---------------------------------------------------------------------------*/
//LOCAL SENSOR_EXTEND_INFO_T g_SR200PC20_ext_info = {
//    (SENSOR_FOCUS_TRIG| SENSOR_FOCUS_ZONE),
//    (SENSOR_EXPOSURE_AUTO|  SENSOR_EXPOSURE_ZONE)
//};

//PUBLIC
SENSOR_INFO_T g_sr200pc20_yuv_info =
{
	SR200PC20_I2C_ADDR_W,                // salve i2c write address
	SR200PC20_I2C_ADDR_R,                 // salve i2c read address
	SENSOR_I2C_FREQ_400,
	...
	s_SR200PC20_resolution_Tab_YUV,    // point to resolution table information structure
	&s_SR200PC20_ioctl_func_tab,        // point to ioctl function table
	PNULL,                            // information and table about Rawrgb sensor
	PNULL,//&g_SR200PC20_ext_info,                // extend information about sensor
	//5,
...
static uint32_t _SR200PC20_GetResolutionTrimTab(uint32_t param)
{
	CMR_LOGI("_SR200PC20_GetResolutionTrimTab\n");
	return (uint32_t) s_SR200PC20_Resolution_Trim_Tab;
}

LOCAL uint32_t _SR200PC20_SetExifInfo_exposure(uint32_t param)
{
	EXIF_SPEC_PIC_TAKING_COND_T *sensor_exif_info_ptr = &s_SR200PC20_exif;
...
LOCAL uint32_t _SR200PC20_SetExifInfo_ISO(uint32_t param)
{
	EXIF_SPEC_PIC_TAKING_COND_T *sensor_exif_info_ptr = &s_SR200PC20_exif;
	uint8_t iso       = 0;
...
{
	SENSOR_PRINT("Start");
	return (uint32_t)&s_SR200PC20_exif;
}
...
LOCAL uint32_t _SR200PC20_InitExifInfo(void)
{
	EXIF_SPEC_PIC_TAKING_COND_T *exif_ptr = &s_SR200PC20_exif;

	memset(&s_SR200PC20_exif , 0, sizeof(EXIF_SPEC_PIC_TAKING_COND_T));
...
*/
/******************************************************************************/
// Description: sensor SR200PC20 power on/down sequence
// Global resource dependence:
// Author: Tim.zhu
// Note:
/******************************************************************************/
LOCAL uint32_t _SR200PC20_PowerOn(uint32_t power_on)
{
	SENSOR_AVDD_VAL_E dvdd_val = g_sr200pc20_yuv_info.dvdd_val;
	SENSOR_AVDD_VAL_E avdd_val = g_sr200pc20_yuv_info.avdd_val;
	SENSOR_AVDD_VAL_E iovdd_val = g_sr200pc20_yuv_info.iovdd_val;
	BOOLEAN power_down = g_sr200pc20_yuv_info.power_down_level;
	BOOLEAN reset_level = g_sr200pc20_yuv_info.reset_pulse_level;

	CMR_LOGI("SENSOR: _SR200PC20_PowerOn (1:on, 0:off): %d \n", power_on);
...
LOCAL uint32_t _SR200PC20_Identify(uint32_t param)
{
	...
	{
		nLoop = 1000;
		CMR_LOGI("anrry:for before sr200pc20\n");

		ret = Sensor_ReadReg(reg[i]);

		CMR_LOGI("anrry:for after sr200pc20\n");
		CMR_LOGI("anrry: sr200pc20 Read reg0x04 = %x\n",ret);
	...
	CMR_LOGI("255: it is SR200PC20\n");

       _SR200PC20_InitExifInfo();
...
LOCAL uint32_t _sr200pc20_StreamOn(uint32_t param)
{
	SENSOR_PRINT("SENSOR: _sr200pc20_StreamOn");
...
LOCAL uint32_t _sr200pc20_StreamOff(uint32_t param)
{
	SENSOR_PRINT("SENSOR: _sr200pc20_StreamOff");
...
LOCAL const SENSOR_REG_T SR200PC20_brightness_tab[][3]=
{
	{
		{0x03, 0x10},
		{0x40, 0xc0},
...
LOCAL uint32_t _SR200PC20_set_brightness(uint32_t level)
{
	...
	SENSOR_REG_T_PTR sensor_reg_ptr=(SENSOR_REG_T_PTR)SR200PC20_brightness_tab[level];
	...
	SENSOR_PRINT("sensor: terry SR200PC20_set_brightness = 0x%02x.\n", level);
	return 0;
}
...
//
/******************************************************************************/
LOCAL const SENSOR_REG_T SR200PC20_saturation_tab[][4]=
{
	{//-3
		{0x03, 0x10},
		{0x61, 0x50},
...
{
	uint16_t i = 0x00;
	uint32_t num_array = sizeof(SR200PC20_saturation_tab)/sizeof(SR200PC20_saturation_tab[0]);

	if(level >= num_array)
	return 0;

	SENSOR_REG_T_PTR sensor_reg_ptr = (SENSOR_REG_T_PTR)SR200PC20_saturation_tab[level];
	...
	SENSOR_PRINT("sensor: terry SR200PC20_set_saturation = 0x%02x.\n", level);
	return 0;
}
...
//
/******************************************************************************/
LOCAL const SENSOR_REG_T SR200PC20_image_effect_tab[][7]=
{
	// effect normal
	{
		{0x03,0x10},
...
LOCAL uint32_t _SR200PC20_set_image_effect(uint32_t effect_type)
{
    ...
    if(effect_type > 7)
    	return 0;
	SENSOR_REG_T_PTR sensor_reg_ptr=(SENSOR_REG_T_PTR)SR200PC20_image_effect_tab[effect_type];
	...
	SENSOR_PRINT("sensor: terry SR200PC20_set_image_effect = 0x%02x.\n", effect_type);
	return 0;
}
...
//
/******************************************************************************/
LOCAL const SENSOR_REG_T SR200PC20_anti_banding_flicker_tab[][3]=
{
...
LOCAL uint32_t _SR200PC20_set_anti_flicker(uint32_t mode)
{
	SENSOR_REG_T_PTR sensor_reg_ptr=(SENSOR_REG_T_PTR)SR200PC20_anti_banding_flicker_tab[mode];
	uint16_t i=0x00;
	...
	SENSOR_PRINT("sensor: terry _SR200PC20_set_anti_flicker = 0x%02x", mode);
...
//
/******************************************************************************/
LOCAL const SENSOR_REG_T SR200PC20_video_mode_tab[][44]=
{
	{//video preview
		{0x03, 0x00},
		{0x01, 0xf9},
...
{//65.6 us
	...
	SENSOR_REG_T_PTR sensor_reg_ptr=(SENSOR_REG_T_PTR)SR200PC20_video_mode_tab[mode];
	uint16_t i=0x00;
	...
	SENSOR_PRINT("sensor: terry _SR200PC20_set_video_mode = 0x%02x", mode);
...
//
/******************************************************************************/
LOCAL const SENSOR_REG_T SR200PC20_awb_tab[][10] =
{
	{//auto
		{0x03, 0x22},
		{0x11, 0x2e},  	//awbctrl2
...
LOCAL uint32_t _SR200PC20_set_awb(uint32_t mode)
{
	...
	SENSOR_REG_T_PTR sensor_reg_ptr=(SENSOR_REG_T_PTR)SR200PC20_awb_tab[mode];
	...
	SENSOR_PRINT("sensor: terry SR200PC20_set_awb = 0x%02x", mode);
	return 0;
}
...
/******************************************************************************/
  LOCAL const SENSOR_REG_T SR200PC20_work_mode_tab[][175]=
{
	{//auto
		{0x03, 0x00},
		{0xa0, 0x00},
...
LOCAL uint32_t _SR200PC20_set_work_mode(uint32_t mode)
{
	...
	SENSOR_REG_T_PTR sensor_reg_ptr=(SENSOR_REG_T_PTR)SR200PC20_work_mode_tab[mode];
...
LOCAL uint32_t _SR200PC20_BeforeSnapshot(uint32_t param)
{
	...
	CMR_LOGE("SR200PC20: SR200PC20_before_snapshot\n");
	if(SENSOR_MODE_PREVIEW_ONE>=param)
	{
	 	return SENSOR_SUCCESS;
	}
	CMR_LOGE("terry  _SR200PC20_BeforeSnapshot =%d.\n",param);

	  _SR200PC20_SetExifInfo_ISO(param);
...
LOCAL uint32_t _SR200PC20_after_snapshot(uint32_t param)
{
	//s_SR200PC20_resolution_Tab_YUV[SENSOR_MODE_PREVIEW_ONE].sensor_reg_tab_ptr = (SENSOR_REG_T*)SR200PC20_preview;
	//s_SR200PC20_resolution_Tab_YUV[SENSOR_MODE_PREVIEW_ONE].reg_count = NUMBER_OF_ARRAY(SR200PC20_preview);
       uint32_t exposure;

	SENSOR_PRINT("terry  _SR200PC20_after_snapshot =%d.\n",param);

    	Sensor_WriteReg(0x03, 0x20);
	exposure=Sensor_ReadReg(0x80);
	exposure=( exposure<<8 ) | Sensor_ReadReg(0x81);
	exposure=( exposure<<8 ) | Sensor_ReadReg(0x82);
	_SR200PC20_SetExifInfo_exposure(exposure);
...
LOCAL uint32_t _ov540_flash(uint32_t param)
{
	...
	if(param)
	{
		/* enable flash, disable in _SR200PC20_BeforeSnapshot */
...
LOCAL uint32_t SR200PC20_InitExt(uint32_t param)
{
	...
	uint32_t            init_table_size = s_SR200PC20_resolution_Tab_YUV[SENSOR_MODE_COMMON_INIT].reg_count;
	SENSOR_REG_T_PTR    p_reg_table = s_SR200PC20_resolution_Tab_YUV[SENSOR_MODE_COMMON_INIT].sensor_reg_tab_ptr;
	...
	SENSOR_PRINT("SENSOR: SR200PC20_InitExt, init_table_size = %d \n", init_table_size);
	...
	while(written_num < init_table_size)
	{
		...
		if(SENSOR_WRITE_DELAY == wr_reg){
			...
			for (i = 0; i < 4; i++) {
				...
				if(ret!=0){
					SENSOR_PRINT("SENSOR: SR200PC20_InitExt, i2c write error, ret=%d \n", ret);
					continue;
				}else{
					break;
				}
	...
	timestamp_new = systemTime(CLOCK_MONOTONIC);
    SENSOR_PRINT("SENSOR: SR200PC20_InitExt time=%d us\n",(timestamp_new-timestamp_old)/1000);

    SENSOR_PRINT("SENSOR: SR200PC20_InitExt, done: ret=%d \n", ret);
...
#define TAIL 2
	...
	uint16_t reg;
	uint16_t value;
       uint8_t reg_addr_value_bits =  g_sr200pc20_yuv_info.reg_addr_value_bits;
	uint32_t		   	alloc_size = 0;
	uint32_t            init_table_size = s_SR200PC20_resolution_Tab_YUV[SENSOR_MODE_COMMON_INIT].reg_count;
	SENSOR_REG_T_PTR    p_reg_table = s_SR200PC20_resolution_Tab_YUV[SENSOR_MODE_COMMON_INIT].sensor_reg_tab_ptr;
...

{
	...
	nsecs_t 			timestamp_old;
	nsecs_t				timestamp_new;
       SENSOR_IOCTL_FUNC_TAB_T_PTR set_ioctl_func_tab_ptr =&s_SR200PC20_ioctl_func_tab;
