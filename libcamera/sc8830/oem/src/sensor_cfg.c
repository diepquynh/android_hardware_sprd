...
SENSOR_INFO_T ** Sensor_GetInforTab(struct sensor_drv_context *sensor_cxt, SENSOR_ID_E sensor_id)
{
...
uint32_t Sensor_GetInforTabLenght(struct sensor_drv_context *sensor_cxt, SENSOR_ID_E sensor_id)
{
	...
	if (AUTO_TEST_CAMERA == at_flag) {
		switch (sensor_id) {
			case SENSOR_MAIN:
				tab_lenght=(sizeof(at_main_sensor_infor_tab)/sizeof(SENSOR_INFO_T*));
				break;

			case SENSOR_SUB:
				tab_lenght=(sizeof(at_sub_sensor_infor_tab)/sizeof(SENSOR_INFO_T*));
				break;

			case SENSOR_ATV:
				tab_lenght=(sizeof(at_atv_infor_tab)/sizeof(SENSOR_INFO_T*));
				break;
