...
#define SENSOR_CTRL_MSG_QUEUE_SIZE             20
...
void sensor_set_cxt_common(struct sensor_drv_context *sensor_cxt)
{
	...
	s_local_sensor_cxt = *(struct sensor_drv_context *)sensor_cxt;
	return;
}

void *sensor_get_dev_cxt(void)
{
	struct sensor_drv_context *p_sensor_cxt = &s_local_sensor_cxt;
...
cmr_int Sensor_Device_WriteRegTab(SENSOR_REG_TAB_PTR reg_tab)
{
	...
	if (0 != ret) {
		CMR_LOGE("failed,  ptr=%x, count=%d, bits=%d, burst=%d ",
			(cmr_u32)reg_tab->sensor_reg_tab_ptr, reg_tab->reg_count, reg_tab->reg_bits, reg_tab->burst_mode);
					...
					SENSOR_I2C_T_PTR i2c_tab)
{
	...
	if (0 != ret) {
		CMR_LOGE("failed, slave_addr=0x%x, ptr=0x%x, count=%d",
			i2c_tab->slave_addr, (cmr_u32)i2c_tab->i2c_data, i2c_tab->i2c_count);
		ret = -1;
	}
...
cmr_int Sensor_WriteI2C(cmr_u16 slave_addr, cmr_u8 *cmd, cmr_u16 cmd_length)
{
	...
	CMR_LOGI("slave_addr=0x%x, ptr=0x%x, count=%d",
		i2c_tab.slave_addr, (cmr_u32)i2c_tab.i2c_data, i2c_tab.i2c_count);
...
cmr_int Sensor_SetMCLK(cmr_u32 mclk)
{
	...
	CMR_LOGI("mclk = %d ", mclk);

	ret = sns_dev_set_mclk(sensor_cxt, mclk);

	CMR_LOGI("Done mclk = %d, ret = %ld ", mclk, ret );
...
cmr_int Sensor_PowerDown(cmr_u32 power_level)
{
	...
	CMR_LOGI("power_level %d", power_level);
...
cmr_int Sensor_SendRegTabToSensor(SENSOR_REG_TAB_INFO_T *sensor_reg_tab_info_ptr)
{
	...
	CMR_LOGI("Sensor_SendRegTabToSensor X.");
...
cmr_int sns_set_id(struct sensor_drv_context *sensor_cxt, SENSOR_ID_E sensor_id)
{
	...
	SENSOR_DRV_CHECK_ZERO(sensor_cxt);
	CMR_LOGI("sensor_id=%d,is_register_sensor=%ld,is_main_sensor=%ld ",
		sensor_id, sensor_cxt->is_register_sensor, sensor_cxt->is_main_sensor);
...
SENSOR_ID_E snr_get_cur_id(struct sensor_drv_context *sensor_cxt)
{
	...
	SENSOR_DRV_CHECK_ZERO(sensor_cxt);
	sensor_register_info_ptr = &sensor_cxt->sensor_register_info;
	CMR_LOGI("snr_get_cur_id,sensor_id =%d", sensor_register_info_ptr->cur_id);
...
cmr_int sns_identify(struct sensor_drv_context *sensor_cxt, SENSOR_ID_E sensor_id)
{
	...
	if (sensor_cxt->sensor_identified && (SENSOR_ATV != sensor_id)) {
		...
		if (0xFF != sensor_index) {
			sensor_info_tab_ptr = (SENSOR_INFO_T **)Sensor_GetInforTab(sensor_cxt, sensor_id);
...
void sns_load_sensor_type(struct sensor_drv_context *sensor_cxt)
{
	...
	if(NULL == fp){
		...
		CMR_LOGI("read sensor param len is %d ", len);
		CMR_LOGI("read sensor param  is %x,%x,%x,%x,%x,%x,%x,%x ",
						...
						SENSOR_ID_E sensor_id)
{
	...
	while(1) {
		...
		if (file_size != raw_fix_info_ptr->lnc.map[i-1][0].len) {
			...
			sensor_cxt->lnc_addr_bakup[index][1] = (cmr_u32)malloc(file_size);
			...
			sensor_cxt->lnc_addr_bakup[index][2] = (cmr_u32)raw_fix_info_ptr->lnc.map[i-1][0].param_addr;	/*save the original address*/
...
cmr_int sns_create_ctrl_thread(struct sensor_drv_context *sensor_cxt)
{
	...
	if (!sensor_cxt->ctrl_thread_cxt.is_inited) {
		...
		message.msg_type  = SENSOR_CTRL_EVT_INIT;
...
cmr_int sns_ctrl_thread_proc(struct cmr_msg *message, void *p_data)
{
	...
	case SENSOR_CTRL_EVT_SETMODE:
		...
		cmr_u32 is_inited = (cmr_u32)message->data;
			...
			SENSOR_EXP_INFO_T **sensor_exp_info_pptr)
{
	...
	CMR_LOGI("info = 0x%x ", (cmr_u32)&sensor_cxt->sensor_exp_info);
...
SENSOR_EXP_INFO_T *Sensor_GetInfo(void)
{
	...
	CMR_LOGI("info=%x ", (cmr_u32)&sensor_cxt->sensor_exp_info);
...
cmr_int snr_set_mark(struct sensor_drv_context *sensor_cxt, cmr_u8 *buf)
{
	...
	CMR_LOGI("sensor_identified=%d,idex is %d,%d.",
